...
import { FileDto } from "../../typings/file";
          ...
          reader.onload = async (evt) => {
            await Axios.default
              .put(data.signedRequest, evt.target?.result, {
                headers: {
                  "Content-Type": isImage ? res.type : selectedFile.type,
                },
              })
              .then(() => {
                helpers.setValue([
                  ...meta.value,
                  { name: "name", size: 2320, resourceURL: data.url },
                ]);
            ...
            <StyledCard variant={"outlined"} key={m.resourceURL}>
              <Row justify={"space-between"} align={"center"}>
                <FlexRow>
                  <Box3>
                    {isImage(m.resourceURL) ? (
                      <img width={56} height={56} src={m.resourceURL} />
                    ) : (
                      <ImFilePdf size={56} color={"#FF505F"} />
                    )}
                  </Box3>
                  <Gutter gap={0.5} />
                  <FlexCol style={{ height: 56 }} justify={"space-between"}>
                ...
                <FlexRow align={"center"}>
                  <Delete
                    onClick={() =>
                      helpers.setValue(addOrRemoveArr(meta.value, m))
                    }
                  />
                  <Gutter gap={1} />
